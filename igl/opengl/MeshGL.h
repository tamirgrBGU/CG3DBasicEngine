...
public:
  ...
  VertexArray vao_mesh;
  VertexArray vao_overlay_lines;
  VertexArray vao_overlay_points;

  VertexBuffer vbo_V; // Vertices of the current mesh (#V x 3)
  VertexBuffer vbo_V_uv; // UV coordinates for the current mesh (#V x 2)
  VertexBuffer vbo_V_normals; // Vertices of the current mesh (#V x 3)
  VertexBuffer vbo_V_ambient; // Ambient material  (#V x 3)
  VertexBuffer vbo_V_diffuse; // Diffuse material  (#V x 3)
  VertexBuffer vbo_V_specular; // Specular material  (#V x 3)

  IndexBuffer vbo_F; // Faces of the mesh (#F x 3)

  IndexBuffer vbo_lines_F;         // Indices of the line overlay
  VertexBuffer vbo_lines_V;         // Vertices of the line overlay
  VertexBuffer vbo_lines_V_colors;  // Color values of the line overlay
  IndexBuffer vbo_points_F;        // Indices of the point overlay
  VertexBuffer vbo_points_V;        // Vertices of the point overlay
  VertexBuffer vbo_points_V_colors; // Color values of the point overlay
  ...
  IGL_INLINE void bind_overlay_points(unsigned int shader_overlay_points,unsigned int shader_id);
  ...
  IGL_INLINE void bind_labels(const TextGL& labels,unsigned int shader_text);
